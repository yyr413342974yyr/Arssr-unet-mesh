...
class _grid_encode(Function):
    ...
    def forward(ctx, inputs, embeddings, offsets, per_level_scale, base_resolution, calc_grad_inputs=False, gridtype=0, align_corners=False, interpolation=0):
        ...
        B, D = inputs.shape # batch size, coord dim       120000,3
        L = offsets.shape[0] - 1 # level  (num_levels)    16
        C = embeddings.shape[1]  # embedding dim for each level    2
        S = np.log2(per_level_scale) # resolution multiplier at each level, apply log2 for later CUDA exp2f   0.511
        H = base_resolution # base resolution      10
        ...
        outputs = torch.empty(L, B, C, device=inputs.device, dtype=embeddings.dtype) #(16,120000,2)
...
class GridEncoder(nn.Module):
    def __init__(self, input_dim=3, num_levels=16, level_dim=2, per_level_scale=2, base_resolution=16, log2_hashmap_size=19, desired_resolution=None, gridtype='hash', align_corners=False, interpolation='linear'):
        ...
        self.num_levels = num_levels # num levels, each level multiply resolution by 2 编码的层级数量，每个层级代表不同的分辨率
        self.level_dim = level_dim   # encode channels per level  每个层级的编码维度
        self.per_level_scale = per_level_scale # multiply resolution by this scale at each level.  每个层级的分辨率缩放因子
        self.log2_hashmap_size = log2_hashmap_size     # 哈希表大小的对数
        self.base_resolution = base_resolution         # 基础分辨率
        self.output_dim = num_levels * level_dim       # 输出的维度
        self.gridtype = gridtype
        self.gridtype_id = _gridtype_to_id[gridtype]   # "tiled" or "hash"
        self.interpolation = interpolation
        self.interp_id = _interp_to_id[interpolation]  # "linear" or "smoothstep"
        self.align_corners = align_corners   # 是否在插值中对齐角点
    ...
    def forward(self, inputs, bound=1):
        ...
        inputs = (inputs + bound) / (2 * bound) # map to [0, 1]   inputs: (120000,3) = (N*K,3)
        ...
        outputs = outputs.view(prefix_shape + [self.output_dim])   # (120000, 32)
